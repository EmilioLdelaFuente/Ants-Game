...
//funciones globales publicas 
	...
	public: 
		...
		double z;
	public: 
		Vector2D(double xv=0.0f,double zv=0.0f):x(xv),z(zv){} 
		Vector2D operator - (const Vector2D v)const {return Vector2D(x-v.x,z-v.z);}   
		Vector2D operator + (const Vector2D v)const {return Vector2D(x+v.x,z+v.z);}    
		double operator *(const Vector2D v)const  {return x*v.x+z*v.z;}       
		Vector2D operator *(double d)const {return Vector2D(x*d,z*d);}    
		double module(){return (double)sqrt(x*x+z*z);}              
		double argument(){return (double)atan2(z,x);}          
		Vector2D unit(){ Vector2D ret(x,z);
			double mod=module();
	...
	inline void Sprite::setPos(float x, float y){pos.x=x;pos.z=y;}
	inline void Sprite::setCenter(float cx, float cy){anchor.x=cx; anchor.z=cy;} 
